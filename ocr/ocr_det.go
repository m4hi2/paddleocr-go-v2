...
func (det *DBDetector) Run(img gocv.Mat) [][][]int {
	...
	predictorInputTensor := det.predictor.GetInputHandle(det.predictor.GetInputNames()[0])
	predictorInputTensor.CopyFromCpu(data)
	det.predictor.Run()
	predictorOutputTensor := det.predictor.GetOutputHandle(det.predictor.GetOutputNames()[0])
	//det.predictor.GetZeroCopyOutput(det.outputs[0])
	predictorOutputTensor.CopyToCpu(det.outputs[0])
